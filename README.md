...
| 백엔드 | Express |
| 프론트엔드 | React 18 + Vite + Tailwind CSS v4 + Zustand |
...
│   │   └── src/
│   │       ├── index.ts                 Express 앱 + 6개 엔드포인트 + 요청 로깅 + 정적 파일 서빙
...
│   │       └── stores/
│   │           ├── sqlite.ts              SQLite DDL(orders + orders_fts + search_log), 시드, FTS5 검색
│   │           └── pinecone.ts            Pinecone 인덱스 upsert, 벡터 검색
│   └── frontend/                      React 좌우 비교 UI
│       └── src/
│           ├── App.tsx                  루트 컴포넌트 (zustand store 연결)
│           ├── store.ts                 zustand 상태 관리 (패널 상태 + 검색 액션 + 캐시)
...
OpenAI 호환 SDK의 baseURL을 Upstage Solar 엔드포인트로 교체하여 임베딩과 LLM 추천을 제공한다. `embed()`는 passage/query 모델을 구분하여 4096차원 벡터를 생성하고, `recommend()`는 검색된 유사 수주 이력을 시스템 프롬프트와 함께 LLM(`solar-pro2`)에 전달하여 대응 방안 3~4가지를 생성한다. 타임아웃 30초, 자동 재시도 1회가 설정되어 있다.

**stores/sqlite.ts** -- SQLite DDL, 시드, FTS5 검색

서버 기동 시 `orders` 테이블, `orders_fts` FTS5 가상 테이블, `search_log` 테이블(관측/평가용)을 생성(DDL)하고, 수주 20건을 INSERT OR IGNORE로 멱등 시드한다. 검색 시 질의문에서 구두점 제거, 공백 분리, 불용어/1글자 필터링으로 키워드를 추출한 뒤 FTS5 MATCH(OR 결합)로 bm25 검색을 수행한다. bm25 원시 점수(음수)를 0~1로 정규화하고, 각 히트에서 실제 매칭된 키워드를 evidence로 첨부한다.
...
6개 API 라우트를 정의한다. `/api/search`(통합 검색), `/api/search/vector`(벡터 단독), `/api/search/rdb`(RDB 단독), `/api/seed`(데이터 적재), `/api/scenarios`(시나리오 반환), `/api/health`(헬스체크). API 키가 없어도 서버가 기동되도록 SolarClient를 조건부로 생성하며, 키 미설정 시 검색 요청에 500 오류를 반환한다. `/api/health`를 제외한 모든 요청은 메서드·경로·상태코드·소요시간을 콘솔에 로깅한다. 프로덕션 빌드(`packages/frontend/dist`)가 존재하면 정적 파일 서빙과 SPA 폴백을 자동으로 활성화한다. 서버 기동 시 SQLite에 수주 20건을 자동 시드한다(멱등).

### 프론트엔드 (`packages/frontend/src/`)

**App.tsx** -- 루트 컴포넌트

zustand의 `useSearchStore`로 벡터/RDB 양쪽 패널 상태와 검색 액션을 구독한다. 초기 렌더링 시 헬스체크를 수행하고 30초 간격으로 연결 상태 뱃지를 갱신한다.

**store.ts** -- zustand 상태 관리

패널 상태(idle/loading/success/empty/error) 정의, 검색 실행(`search`), 재시도(`retry`), 헬스체크(`refreshHealth`) 액션을 하나의 zustand store로 관리한다. `panelResultToState` 함수가 API의 PanelResult를 프론트엔드 PanelState로 변환(success/empty/error 분기)한다. 동일 질의 재요청 방지를 위한 클라이언트 캐시(Map)를 store 외부에 유지하고, 패널별 독립 상태 업데이트로 실패 격리(NFR-09)를 보장한다.
